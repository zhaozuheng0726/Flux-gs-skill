...
If `tmc3` is not in `PATH`, update the path in `utils/gpcc_utils.py`.
...
Run training from the Flux-GS repository root.
...
The WebGL viewer lives in a separate static project, for example:
...
The scene-level `main.js` only selects which model file to load. The default camera path and interaction logic are in `render_shared/main.js`. For a new scene, the model will still load, but the initial view may not frame the object perfectly. Use the mouse/keyboard in the viewer to find a good view, then tune the shared camera setup later if you need a polished public demo.

## 6. Run The Web Demo

Start a static HTTP server from the WebGL project root:
...
Some scenes need small tuning. Start from these examples:
